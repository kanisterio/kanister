...
func (b *PhaseApplyConfiguration) WithState(value crv1alpha1.State) *PhaseApplyConfiguration {
	b.State = &value
	return b
}
...
func (b *PhaseApplyConfiguration) WithOutput(entries map[string]any) *PhaseApplyConfiguration {
	if b.Output == nil && len(entries) > 0 {
		b.Output = make(map[string]any, len(entries))
	}
