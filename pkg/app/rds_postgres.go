...
func (pdb *RDSPostgresDB) Install(ctx context.Context, ns string) error {
	...
	_, err = rdsCli.CreateDBInstance(ctx, 20, dbInstanceType, pdb.id, pdb.dbSubnetGroup, "postgres", pdb.username, pdb.password, []string{pdb.securityGroupID}, pdb.publicAccess)
	if err != nil {
		return err
	}
