...
type Controller struct {
	config    *rest.Config
	crClient  *crv1alpha1.CRV1alpha1Client
...
func (c *Controller) onAdd(obj interface{}) {
	...
	case *crv1alpha1.ActionSet:
		if err := c.onAddActionSet(v); err != nil {
			log.Errorf("Callback onAddActionSet() failed")
			fmt.Printf("Stack: %+v", err)
		}
	case *crv1alpha1.Blueprint:
		if err := c.onAddBlueprint(v); err != nil {
			log.Errorf("Callback onAddBlueprint() failed")
			fmt.Printf("Stack: %+v", err)
		}
	default:
...
func (c *Controller) onDelete(obj interface{}) {
	...
	case *crv1alpha1.ActionSet:
		if err := c.onDeleteActionSet(v); err != nil {
			log.Errorf("Callback onDeleteActionSet() failed: %s", err.Error())
		}
	case *crv1alpha1.Blueprint:
		if err := c.onDeleteBlueprint(v); err != nil {
			log.Errorf("Callback onDeleteBlueprint() failed: %s", err.Error())
		}
	default:
...
func (c *Controller) runAction(ctx context.Context, as *crv1alpha1.ActionSet, aIDX int) error {
	...
	tp, err := kanister.NewTemplateParams(ctx, c.clientset, action)
	if err != nil {
		return err
	}
	artTpls := as.Status.Actions[aIDX].Artifacts
	arts, err := kanister.RenderArtifacts(artTpls, *tp)
	if err != nil {
		return err
	}
