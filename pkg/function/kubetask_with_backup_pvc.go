...
func (f *kubeTaskWithBackupPVCFunc) parseBackupCoreArgs(args map[string]interface{}, parsed *kubeTaskWithBackupPVCArgs) error {
	...
	return OptArg(args, KubeTaskWithBackupPVCKeepPVCOnFailureArg, &parsed.keepPVCOnFailure, false)
}
...
func (f *kubeTaskWithBackupPVCFunc) parseBackupSnapshotArgs(args map[string]interface{}, parsed *kubeTaskWithBackupPVCArgs) error {
	...
	// Mirror Validate() so unit tests bypassing Validate still fail loud.
...
func (f *kubeTaskWithBackupPVCFunc) run(ctx context.Context, cli kubernetes.Interface, a *kubeTaskWithBackupPVCArgs) (out map[string]interface{}, retErr error) {
	...
	// LIFO: this fires BEFORE the PVC cleanup so the mount is released first.
	defer func() {
...
func (f *kubeTaskWithBackupPVCFunc) takeStagingSnapshot(
	...
	vs, err := createCSISnapshot(ctx, snapshotter, snapName, a.namespace, pvcName, a.snapshotClass, true, snapLabels)
	if err != nil {
		return nil, err
	}
...
// deriveRestoreSize falls back from VS RestoreSize → PVC status capacity →
// PVC spec request → default, so the output is never empty for streaming
// drivers that leave VS RestoreSize nil.
...
// alive past command exit so the CSI driver still sees the mount at
// CreateSnapshot time. Returns (parsed output, pod name, error); pod deletion
...
// wrapCommandForKeepAlive wraps the user command so that after it exits the
// pod prints a marker line with the exit code and sleeps. Only supports
...
// waitForKeepAliveMarker scans pod logs until the marker arrives, returning
// the embedded exit code and everything emitted before the marker.
