...
func (s *RDSFunctionsTest) TestPrepareCommand(c *check.C) {
	testCases := []struct {
		...
		`,
					"db-endpoint",
					`{\"Location\":{\"type\":\"\",\"bucket\":\"\",\"endpoint\":\"\",\"prefix\":\"\",\"region\":\"\"},\"Credential\":{\"Type\":\"\",\"KeyPair\":null,\"Secret\":null,\"KopiaServerSecret\":null},\"SkipSSLVerify\":false}`,
					fmt.Sprintf("%s/%s", "/backup/postgres-backup", "backup-id"),
					postgres.DefaultConnectDatabase),
			},
			tp: param.TemplateParams{
