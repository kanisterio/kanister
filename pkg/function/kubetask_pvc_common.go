...
// decoder output) into map[string]interface{} so encoding/json can handle it.
...
// arg-parser drops embedded LocalObjectReference inside valueFrom.secretKeyRef
// because of how mapstructure handles JSON `,inline` tags, so we re-decode
// via json to preserve the nested fields.
...
// cleanup runs even when the caller's ctx is cancelled.
