...
func (s *RepoServerControllerSuite) createRepositoryServerSecrets(c *C) {
	...
	s.repoServerSecrets.repositoryPassword, err = s.CreateRepositoryPasswordSecret(testutil.GetRepoPasswordSecretData(testutil.KopiaRepositoryServerAdminPassword))
	c.Assert(err, IsNil)
