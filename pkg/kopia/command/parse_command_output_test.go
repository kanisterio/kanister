	...
	for _, tc := range []struct {
	...
	}{
		{
		   ...
		   ` + `{"id":"00000000000000000000001","source":{"host":"h2","userName":"u2","path":"/tmp/aaa1"},` +
				`"description":"","startTime":"2021-05-26T05:29:07.206854927Z","endTime":"2021-05-26T05:29:07.207328392Z",` +
				`"rootEntry":{"name":"aaa1","type":"d","mode":"0755","mtime":"2021-05-19T15:45:34.448853232Z",` +
				`"obj":"ka68ba7abe0818b24a2b0647aeeb02f29","summ":{"size":0,"files":1,"symlinks":0,"dirs":1,` +
				`"maxTime":"2021-05-19T15:45:34.448853232Z","numFailed":0}}}` + `
		   `,
			checker:        check.IsNil,
...
func (kParse *KopiaParseUtilsTestSuite) TestErrorsFromOutput(c *check.C) {
	// Colored ERROR message bytes
	coloredErrorBytes := []byte{
		27, 91, 51, 49, 109, 69, 82, 82, 79, 82, 27, 91, 48, 109, 32, 111, 112, 101, 110, 32, 114, 101, 112, 111, 115, 105, 116, 111, 114, 121, 58, 32,
	...
	for caseIdx, tc := range []struct {
		...
		{string(coloredErrorBytes), []string{"open repository: repository is not connected."}},
