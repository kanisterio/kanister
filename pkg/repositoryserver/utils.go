...
func repoServerPodService(namespace string, repoServerCROwnerRef metav1.OwnerReference) *corev1.Service {
...
func getPodOverride(ctx context.Context, kubeCli kubernetes.Interface, namespace string) map[string]interface{} {
	podName := os.Getenv("HOSTNAME")
	pod, err := kubeCli.CoreV1().Pods(namespace).Get(ctx, podName, metav1.GetOptions{})
	if err != nil {
		log.WithError(err).Print("Could not get pod spec")
		return nil
	}
	uidguid := int64(0)
	nonRootBool := false
