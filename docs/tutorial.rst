...
Whenever we create a blueprint resource, it goes through a
...
controller that does some basic checks on the resource. The things that are
checked can be found by referring to :ref:`kanctlvalidate` document.
