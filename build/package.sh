...
docker buildx build --load --pull --sbom=${GENERATE_SBOM:-false} ${baseimagearg:-} --build-arg kanister_version=${VERSION} -t ${IMAGE}:${VERSION} --platform linux/${ARCH}  -f .dockerfile-${ARCH} .
