...
```bash
helm install --name myrelease --namespace kanister stable/kanister-operator --set image.tag=0.14.0
```
...
tool called `kanctl` used to help manage Kanister CRDs.  The `kanctl` binary can be
