...
func (v *VulnerabilityParserSuite) TestNotExistentResult(c *C) {
	...
	matchingVulnerabilities, err := parseVulerabilitiesReport("mockData/result_non_existent.json", severityLevels)
...
func (v *VulnerabilityParserSuite) TestInvalidJson(c *C) {
	severityLevels := []string{"High", "Critical"}
	matchingVulnerabilities, err := parseVulerabilitiesReport("mockData/results_invalid.json", severityLevels)
	c.Assert(len(matchingVulnerabilities), Equals, 0)
	c.Assert(err, NotNil)
}
...
func (v *VulnerabilityParserSuite) TestValidJsonWithZeroVulnerabilities(c *C) {
	severityLevels := []string{"High", "Critical"}
	matchingVulnerabilities, err := parseVulerabilitiesReport("mockData/results_valid_no_matches.json", severityLevels)
	c.Assert(len(matchingVulnerabilities), Equals, 0)
	c.Assert(err, IsNil)
}
...
func (v *VulnerabilityParserSuite) TestValidJsonForLowVulerabilities(c *C) {
	...
	matchingVulnerabilities, err := parseVulerabilitiesReport("mockData/results_valid.json", severityLevels)
	c.Assert(len(matchingVulnerabilities), Equals, 0)
	c.Assert(err, IsNil)
}
